...
def test_get_beverage_by_id_service(client, create_beverage, beverage_uri):
    ...
    pytest.assume(response.status.startswith('200'))
    returned_current = response.json
    for param, value in current_beverage.items():
        pytest.assume(returned_current[param] == value)

def test_get_sizes_service(client, create_beverages, beverage_uri):
    response = client.get(beverage_uri)
    pytest.assume(response.status.startswith('200'))
    returned_sizes = {beverage['_id']: beverage for beverage in response.json}
    for beverage in create_beverages:
        pytest.assume(beverage['_id'] in returned_sizes)